...
public class Discord {
    public static void succesWebhook() {
        try {
    ...
    public static void failWebhook(String str) {
        try {
            URI uri = URI.create(Globalvariable.Discord_webhook_url_error);
            URL url = uri.toURL();
            ...
            //전송할 메시지
            String message = Globalvariable.datetime + ", " + Globalvariable.start_station + "-" + Globalvariable.end_station + " 티켓에러: " + str;
            String jsonPayload = String.format("{\"content\": \"%s\"}", message);
